...
function Channels() {
  ...
  const defaultFormData: Partial<Channel> = {
  ...
  const [formData, setFormData] = useState<Partial<Channel>>(defaultFormData)
  const [formError, setFormError] = useState<string | null>(null)
  const [formLoading, setFormLoading] = useState<boolean>(false)

  const { confirm, ConfirmDialog } = useConfirm()

  const openDialog = (mode: DialogMode, a?: Channel) => {
    if (mode === "edit" && a) {
  ...
  const handleDelete = async (channelId: string) => {
    const ok = await confirm({
      ...
      description:
        "Deleting a channel will permanently delete all associated broadcast data including vods managed by this app.",
    })
    if (!ok) return
    try {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8">
        ...
        <Button className="mt-6" onClick={refetchStreamers}>
          Retry
        </Button>
      </div>
    )
      ...
      {!selectedStreamer.channels || selectedStreamer.channels.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-4">
          {selectedStreamer.channels.map((channel) => (
            <div key={channel.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow max-w-md">
              <div className="flex items-start justify-between">
                <div className="space-y-1 flex-1">
                  ...
                  <p className="flex flex-row gap-2 text-sm text-muted-foreground">
                    Platform: {channel.platform} {getPlatformIcon(channel.platform)}
                  </p>
                  <p className="text-xs text-muted-foreground font-mono mt-2 break-all">Last Streamed: {null}</p>
                </div>
